...
def knn_graph_constrained(df, k,node_ids,cl_pairs,ml_pairs,lambda_cl,eta_ml, verbose=False):
    points = [p[1:] for p in df.itertuples()]
    g = nx.Graph()
    for i in range(0, len(points)):
        g.add_node(i)
    ...
    for i, p in iterpoints:
        distances = list(map(lambda x: euclidean_distance(p, x), points))
        for p1, p2 in cl_pairs:
            if node_ids[i] == p1:
                p2_idx = node_ids.index(p2)
                distances[p2_idx] += lambda_cl
            elif node_ids[i] == p2:
                p1_idx = node_ids.index(p1)
                distances[p1_idx] += lambda_cl
        for p1, p2 in ml_pairs:
            if node_ids[i] == p1:
                p2_idx = node_ids.index(p2)
                distances[p2_idx] = eta_ml
            elif node_ids[i] == p2:
                p1_idx = node_ids.index(p1)
                distances[p1_idx] = eta_ml
        closests = np.argsort(distances)[1:k+1]  # second trough kth closest
        # print(distances[0])
