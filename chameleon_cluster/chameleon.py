from graphmb.chameleon_cluster.graphtools import *
import itertools
import pandas as pd
...
def cluster_constrained(df,node_ids,cl_pairs,ml_pairs, k, knn=10, m=30, alpha=2.0, verbose=False,eta_ml=1e-10 , lambda_cl = 1000000, plot=False):
    #here, we additionally receive the node ids, and the pairs list
    #we change the knn part of the graph to include restrictions on which connections
    graph = knn_graph_constrained(df,k=knn,node_ids=node_ids,ml_pairs=ml_pairs,cl_pairs=cl_pairs,eta_ml=eta_ml,lambda_cl=lambda_cl, verbose=True)
    graph = pre_part_graph(graph, m, df, verbose=True)
    iterm = tqdm(enumerate(range(m - k)), total=m-k)
    for i in iterm:
        merge_best(graph, df, alpha, k, verbose)
