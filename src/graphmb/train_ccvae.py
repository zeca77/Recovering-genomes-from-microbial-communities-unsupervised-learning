...
def prepare_data_for_gnn(
    ...
    if binarize:
        ...
        print(
            f"using this threshold ({percentile} percentile) {threshold} on adj matrix with {len(dataset.adj_matrix.row)} edges")
    ...
    if remove_same_scg:
        edges_with_same_scgs, _ = dataset.get_edges_with_same_scgs()
...
def run_model_ccvae(dataset, args, logger, nrun, epochs=None,
    ...
    with mlflow.start_run(run_name=args.assembly.split("/")[-1] + "-" + args.outname):
        ...
        if plot:
            id_to_scg = {i: set(dataset.contig_markers[node_name].keys()) for i, node_name in
                         enumerate(dataset.node_names)}
        ...
        if args.eval_split == 0:
            ...
            if "node" in args.batchtype:
                train_idx = np.array(random.sample(list(range(gnn_model.adj.indices.shape[0])),
                                                   int(gnn_model.adj.indices.shape[0] * (1 - args.eval_split))))
                ...
                eval_idx = np.array([x for x in np.arange(gnn_model.adj.indices.shape[0]) if x not in train_idx])
            logging.info(
                f"**** using {gnn_model.adj.indices.shape[0]} for training and {gnn_model.adj.indices.shape[0]} for eval")
        ...
        for e in pbar_epoch:
            ...
            if (e + 1) % RESULT_EVERY == 0 and e > args.evalskip and target_metric != "noeval":
                ...
                if args.quiet:
                    ...
                    logger.info(
                        f"[{args.outname} {nlayers_gnn}l {pname}]{losses_string} {scores_string} GPU={gpu_mem_alloc:.1f}MB")
